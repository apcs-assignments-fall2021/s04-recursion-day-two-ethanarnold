public class MyMain {
    ...
    public static int countBigDigits(int x) {
        ...
        if (x % 10 >= 5) return 1 + countBigDigits(x / 10);
    ...
    public static boolean moreOddThanEvenTR(int x, int odd, int even) {
        if (x == 0) return (odd > even);
        else {
            int lastDigit = x % 10;
            if (lastDigit % 2 != 0) //if last digit is odd
            {
    ...
    public static String parenTrim(String str) {
        if (str.charAt(0) == '(' && str.charAt(str.length()-1) == ')') {
            ...
            if (str.charAt(0) == '(') return parenTrim(str.substring(0, str.length()-1));
            if (str.charAt(str.length()-1) == ')') return parenTrim(str.substring(1));
            return parenTrim(str.substring(1, str.length()-1));
        }
    }

    // This method returns the original string reversed;
    // tail recursion could be helpful, but it is not required!
    // Examples:
    ...
    public static String reverseTR(String str, String reverse) {
        if(str.isEmpty()) return reverse;

        reverse += str.charAt(str.length()-1);
        return reverseTR(str.substring(0, str.length()-1), reverse);
    }



    public static void main(String[] args) {
        System.out.println(numDigits2(19283));
    }
}
